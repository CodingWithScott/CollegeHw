...
#	reject = False		# flag to indicate end-of-loop behavior, current route through machine ends in Reject
	...
	#while clients.get(s, 0) != 0 and index < len(clients[s].buffstr)-1:
	while index < len(clients[s].buffstr)-1:
		...
		if clients[s].buffstr[index] == "":			# reach end of current buffer, done reading current message
		...
		elif clients[s].state == "chat_accept":
			...
			if clients[s].buffstr[index] == ")":
				...
				print("reject join6")
		elif clients[s].state == "join_accept":
			print("join_accept")
			...
			if clients[s].buffstr[index] == ")":
				...
				clients[s].state = "resync1"
				print("reject stat4")
		elif clients[s].state == "stat_accept":
