...
Implement DOS file extension naming mechanism 
	...
	Low priority
Change manual string parsing to regualr expressions
...
########################################################################
# Most important function: giving strikes and kicking users
def givestrike(s, reason):
	if strikes.get(s, 0) == 2:
		strikes.pop(s, None)
		names.pop(s, None)
...
def cjoin(s, data):
	...
	def try_add(input_name):
		...
		DOSname = DOSname.replace("\"", "")
		DOSname = DOSname.replace("\\", "")
		# I don't think this contrl chars filter works...
...
def cchat(s, data):
	...
	cchat_regex = re.compile(r'[(](cchat)[(](.*)[)][(](.*)[)][)]')
	str = "(cchat(JOE,MATT,SCOTTY)(hello guys!))"

	# cchat_regex.match(data) is an object containing different groups 
	# of data and other methods. Is very cool.	 
	ppl = cchat_regex.match(data).group(2)	# ex:  'MOE,LARRY,CURLY'
	ppl_list = ppl.split()					# list format of recipients
	msg = cchat_regex.match(data).group(3)	# ex:  'yuk yuk yuk!'
	...
	if ppl == "all":
		...
		for curr_sock in names:
			curr_sock.send(msg)
	elif ppl == "any":
		# need to randomly choose an entry from hash table
		sys.std.out("Pretend the server random messaged:  " + msg)
	else:
		# search hashtable of all names for matches
		print("*** Connection terminated ***")
		for curr_sock in names:
			# send msg when the curr_sock name is in ppl_list
			if names.get(curr_sock) in ppl_list:
				curr_sock.send(msg)
