		...
		# strike if there's >1 dot remaining
			...
			if name_exists(clients[s].my_fullname):
				...
				while name_exists(clients[s].my_fullname):
					...
					incr_name(s)
				print("new incremented name:\t" + clients[s].my_fullname)
...
#	almostkick = False	# set to true if user is getting kicked, then clear data, then kick
	...
	#while kick == False and clients.get(s, 0) != 0 and index < len(clients[s].buffstr):
	while clients.get(s, 0) != 0 and index < len(clients[s].buffstr):
		index += 1
		line_info = "x: " + str(index) 
		line_info += "\t\tdata[" + str(index) 
		line_info += "]:  " + clients[s].buffstr[index-1]
		#print(line_info)

		print("curr state:\t" + clients[s].state)
...
#				clients[s].buffstr = clients[s].buffstr[index+1:]
		...
		if reject:
			print("Invalid command\t\t" + clients[s].buffstr[:index+1] + "\n\n")
#			print("resyncing to eliminate bad input")
			clients[s].buffstr = clients[s].buffstr[index+1:]		# resync, cut out bad input
...
#			print("Valid command:\t\t" + clients[s].buffstr[:index+1])
			clients[s].buffstr = clients[s].buffstr[index+1:]		# resync, accept 
			#print("new buffer:  " + clients[s].buffstr)
			accept = False
			index = -1		# why the fuck is this -1? I guess it's getting incremented somewhere, fuck if i know
			clients[s].state = "start"
			if command == "chat":
				print("line 701")
		...
		# Perform kick check at end if client accrued 3 strieks total
		if clients[s].strikes >= 3:
			kick_user(s)
