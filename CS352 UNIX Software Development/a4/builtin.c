/*	$Id: builtin.c,v 1.17 2014/11/04 08:41:36 felchs Exp $	*/
...
void sstat		(char **argv, int argc);
void unshift	(char **argv, int argc);
...
int exec_builtin(char **argv, int argc) {
	...
	the_builtins[5].name = "shift";
	the_builtins[5].funcptr = shift;
	the_builtins[6].name = "sstat";
	the_builtins[6].funcptr = sstat;
	the_builtins[7].name = "unshift";
	the_builtins[7].funcptr = unshift;
...
void my_exit (char **argv, int argc) {
	...
	if (argc == 1)
		...
		fprintf (stderr, "Usage: exit [optional integer]\n");
		last_exit_val = 1;
		return;
	}
	last_exit_val = 0;
