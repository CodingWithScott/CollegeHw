//Note: Does not compile, but we gave it a shot.
...
void rotate(int dim, pixel *src, pixel *dst) 
{
	...
	int counter;	// It's a counter, deal with it
	
	for (counter = 0; counter < 4; counter++) 
		u.my_numbers[counter] = 0;
		
	for (counter = 0; counter < 4; counter++) {
		printf("%d", u.my_numbers[counter]); 
		printf("\n");
	}
...
//	destination = dst;		
	...
	//u.my_numbers = {1,2,3,4}; 	// Vectors used to access original pointer, and also 1, 2, and 3 locations away at the same time

	int i, j;
	
//	u.dest = u.dest & 0;	// Set pointer to all 0s
...
//			dst[dim_mak+vec]    = src[(i+ 0 + vec)*(dim)+(j)];
			*u.destination     = src[(i+ 0)*(dim)+(j)];
			*u.(destination+1) = src[(i+ 1)*(dim)+(j)];
			*u.(destination+2) = src[(i+ 2)*(dim)+(j)];
			*u.(destination+3) = src[(i+ 3)*(dim)+(j)]; 
			
/*			*(destination  )   = src[(i+ 0 + vec)*(dim)+(j)];
			*(destination+1) = src[(i+ 1 + vec)*(dim)+(j)];
			*(destination+2) = src[(i+ 2 + vec)*(dim)+(j)];
			*(destination+3) = src[(i+ 3 + vec)*(dim)+(j)];    */
...
void smooth(int dim, pixel *src, pixel *dst) 
{
	...
	union u_tag {				// Union type is called "u_tag"
		v4si vector;			// Vector used for performing SIMD operations
		unsigned short my_numbers[3];	// Array of 3 unsigned shorts for subpixels
		pixel *destination;			// Will be a copy of the source pixel passed in, aligned to first part of union
	} pix;						// Union of type "u_tag" is called "pix";
