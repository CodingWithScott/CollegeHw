...
;(define (make-clean-copy-animal)
;  (let ((infile (open-input-file "animalfarm.txt"))
; (let ((infile (open-input-file inputname))
;      (outfile (open-output-file "animalfarmclean.txt"
;      (outfile (open-output-file outputname
;                                 #:mode 'text 
;                                 #:exists 'replace)))
;  (let loop ((next-char (read-char infile)))
...
(define (fill-table filename ht)
  (display "fill-table called, filename:  ") (display filename) (newline)
...
;(define (fill-table-animal)
;  (display "fill-table-animal called  ") (newline)
;  (let ((infile (open-input-file "animalfarmclean.txt")))
  ;(let ((infile (open-input-file filename)))
;  (let loop ((next-char (read-char infile)))
;    (when (not (eof-object? next-char))
;      (when (char-alphabetic? next-char)
;          (display next-char)
;          (hash-set! plain-ht next-char (+ (hash-ref plain-ht next-char) 1)))
;      (loop (read-char infile))))
;  (close-input-port infile)))

;(define (fill-table-1984)
;  (display "fill-table-1984 called  ") (newline)
;  (let ((infile (open-input-file "1984clean.txt")))
  ;(let ((infile (open-input-file filename)))
;  (let loop ((next-char (read-char infile)))
    ;(define curr_char (next-char (read-char infile)
;    (when (not (eof-object? next-char))
;      (when (char-alphabetic? next-char)
;          (display next-char)
;        next-char
;          (hash-set! 1984-ht next-char (+ (hash-ref 1984-ht next-char) 1)))
;      (loop (read-char infile))))
;  (close-input-port infile)))))
...
;(make-clean-copy "1984.txt" "1984clean.txt")
;(make-clean-copy-animal)
