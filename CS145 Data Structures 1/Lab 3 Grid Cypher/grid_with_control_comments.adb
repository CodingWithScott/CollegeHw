...
--			put("Min = "); put(min); new_line;	-- Output the data read in from Arguments 1-3 (will probably delete these later)
--			put("Max = "); put(max); new_line;
			put("Filename = "); put(Argument(3)); new_line;
--			Put("Cypher length:  "); put(cypher_string.length); new_line;
--			Put("Cypher text:  "); put(cypher_string); new_line;
	...
	procedure Decode(width : in positive) is
		---------------- Varying 2-D Array ---------------
		type Varrying_Array is array(Positive range <>, Positive range <>) of Character;
	...
	begin
		declare 
			Matrix : Varrying_Array(1..curr_length, 1..curr_width);
		begin
--			put("Curr_width = "); put(curr_width); new_line;
--			put("Curr_length = "); put(curr_length); new_line;
--			put("Cypher_string.length = "); put(cypher_string.length); new_line; -- Put("(includes EOF character)"); new_line;
--			put("Remainder = "); put(remainder); new_line;
--			put("Cypher_string = "); put(cypher_string); new_line;
...
--						put(cypher_string.value(char_counter)); put("   Character # = "); put(char_counter); new_line;
						if char_counter < (cypher_string.length + 1) then-- and cypher_string.value(char_counter) /= ' ' then
...
--						put(Matrix(Row, Column)); --put("   char_counter = "); put(char_counter); new_line;
--						char_counter := char_counter + 1;
--					end if;
--				end loop;
--				new_line;
...
--			Put_Line("This is the decoded string...");
			char_counter := 1;				-- Reset char_counter to avoid printing out garbage characters
			for Row in 1 .. curr_length loop
				for Column in 1 .. curr_width loop
					if char_counter < (cypher_string.length + 1) and Matrix(Row, Column) /= ' ' then
