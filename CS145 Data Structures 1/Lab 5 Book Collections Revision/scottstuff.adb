-- Scott Felch
-- My own version of the program Bookstuff, for just testing out functionality I have so far
...
--		put_line("Get_book_info called successfully");
	...
	begin
		...
		Close(book_file);
	exception
		when Collection_full =>
			Put_line("The collection is full.  No more books can be added");	
...
--	for N in 1 .. Size(collection1) loop
--		Testbookstring := ToString(collection1.book_array(N));
--		put(testbookstring); new_line;
--	end loop;
-- 	put(collection1, "First collection");
...
exception
	...
	when Collection_Full =>
		Put_Line("Aww shoot, that collection is completely full! I couldn't add any books to it. Bummer, bro. :(");
